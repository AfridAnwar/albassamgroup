...
jQuery(document).ready(function ($) {
  ...
  var preloader = function () {
    if (!$("body").hasClass("fp-site-preloader")) {
      $(
        ".blog a.post-image>img, .post.single .post-image img, .services-list a>img, .projects-list:not('.isotope') a>img, .fp-preload>img"
      ).each(function () {
  ...
  $(document).on(
    "click",
    ".search-overlay .search-absolute-container",
    function (event) {
  ...
  $(".mobile-menu-switch").on("click", function (event) {
    event.preventDefault();
    $(
      ".header-container:not('#fp-sticky-clone') .mobile-menu-container"
    ).appendTo("body");
    $("body").addClass("overlay");
    $(".background-overlay").css("z-index", 2);
    if (!$(".mobile-menu-container nav .mobile-menu").is(":animated")) {
      $(".mobile-menu-container nav .mobile-menu").toggle(
        "slide",
        { direction: "right" },
        500
      );
  ...
  $(document).on(
    "click",
    ".mobile-menu .template-big-arrow-horizontal-sm, .overlay .background-overlay",
    function (event) {
      event.preventDefault();
  ...
  $(".collapsible-mobile-submenus .template-plus3").on(
    "click",
    function (event) {
      event.preventDefault();
  ...
  $(".cost-slider").each(function () {
    $(this)
      .slider({
        ...
        slide: function (event, ui) {
          ...
          $(this)
            .find(".cost-slider-tooltip .value")
            .html(
              (typeof $(this).data("currencybefore") != "undefined"
                ? $(this).data("currencybefore")
                : "") +
                ui.value
                  .toFixed()
                  .replace(
                    regex,
                    "$&" +
                      (typeof $(this).data("thousandthseparator") != "undefined"
                        ? $(this).data("thousandthseparator")
                        : "")
                  ) +
                (typeof $(this).data("currencyafter") != "undefined"
                  ? $(this).data("currencyafter")
                  : "")
            );
          $(this)
            .find(".cost-slider-tooltip")
            .css(
              "left",
              "-" +
                Math.round(
                  ($(this).find(".cost-slider-tooltip .value").outerWidth() -
                    21) /
                    2
                ) +
                "px"
      ...
      .append(
        '<div class="cost-slider-tooltip"><div class="value">' +
          (typeof $(this).data("currencybefore") != "undefined"
            ? $(this).data("currencybefore")
            : "") +
          $(this)
    ...
    if (sliderTooltip.is(":visible"))
      sliderTooltip.css(
        "left",
        "-" +
          Math.round((sliderTooltip.children(".value").outerWidth() - 21) / 2) +
          "px"
      );
  });
  ...
  $(".cost-dropdown").each(function () {
    $(this).selectmenu({
      ...
      create: function (event, ui) {
        ...
        $("#" + $(this).attr("id") + "-menu")
          .parent()
          .addClass("cost-dropdown-menu")
          .addClass("cost-dropdown-menu-" + $(this).closest("form").attr("id"));
        if ($(this).closest("form").hasClass("style-simple")) {
          $("#" + $(this).attr("id") + "-menu")
            .parent()
            .addClass("cost-dropdown-menu-style-simple");
        }
        $("#" + $(this).attr("id") + "-menu")
          .parent()
          .addClass("cost-dropdown-menu");
  ...
  $("#monthly-repayments").costCalculator({
    formula:
      "loan-amount*{powerstart}1+rate-of-interest/100/12^loan-term{powerend}*(1+rate-of-interest/100/12{-}1)/({powerstart}1+rate-of-interest/100/12^loan-term{powerend}{-}1)",
    currencyBefore: "$",
    thousandthSeparator: ",",
    decimalSeparator: "",
    decimalPlaces: 0,
  ...
  $("#startup").costCalculator({
    formula:
      "sales-per-month-value*business-type+purchase-per-month-value+employees-number*30+foreign-currency*sales-per-month-value+employees-number*annual-tax+sales-per-month-value*invoicing-customers+tax-advisor+tax-optimization+contracts-preparation",
    currencyBefore: "$",
    currencyAfter: "/ month",
    thousandthSeparator: ",",
    decimalSeparator: ".",
  ...
  $("#timesaver").costCalculator({
    formula:
      "sales-per-month-value*(business-type+business-type*1.85)+purchase-per-month-value+employees-number*35+foreign-currency*sales-per-month-value*1.5+employees-number*annual-tax+sales-per-month-value*invoicing-customers+tax-advisor+tax-optimization+contracts-preparation",
    currencyBefore: "$",
    currencyAfter: "/ month",
    thousandthSeparator: ",",
    decimalSeparator: ".",
  ...
  $("#enterprise").costCalculator({
    formula:
      "sales-per-month-value*(business-type+business-type*5.65)+purchase-per-month-value*1.2+employees-number*45+foreign-currency*sales-per-month-value*2+employees-number*annual-tax+sales-per-month-value*invoicing-customers+tax-advisor+tax-optimization+contracts-preparation",
    currencyBefore: "$",
    currencyAfter: "/ month",
    thousandthSeparator: ",",
    decimalSeparator: ".",
  ...
  $("#larger-loan").costCalculator({
    formula:
      "2*loan-amount*{powerstart}1+larger-rate-of-interest/100/12^loan-term{powerend}*(1+larger-rate-of-interest/100/12{-}1)/({powerstart}1+larger-rate-of-interest/100/12^loan-term{powerend}{-}1)",
    currencyBefore: "$",
    currencyAfter: "/ month",
    thousandthSeparator: ",",
    decimalSeparator: "",
  ...
  $("#longer-loan").costCalculator({
    formula:
      "loan-amount*{powerstart}1+longer-rate-of-interest/100/12^(2*loan-term){powerend}*(1+longer-rate-of-interest/100/12{-}1)/({powerstart}1+longer-rate-of-interest/100/12^(2*loan-term){powerend}{-}1)",
    currencyBefore: "$",
    currencyAfter: "/ month",
    thousandthSeparator: ",",
    decimalSeparator: "",
  ...
  } else $(".fp-parallax").addClass("attachment-scroll");
              ...
              if (
                ...
                if (direction == "left") {
                  ...
                  if (base == "x" || base == 0) {
                    ...
                    base =
                      -self.data("scroll") * self.children().first().width() -
                      self.data("scroll") *
                        parseInt(
  ...
  var horizontalCarousel = function () {
    $(".horizontal-carousel").each(function (index) {
      ...
      $(".fp-preloader-hr-carousel_" + index).before(
        "<span class='fp-preloader'></span>"
      );
      $(".fp-preloader-hr-carousel_" + index).imagesLoaded(function (instance) {
        $(".fp-preloader-hr-carousel_" + index)
          .prev(".fp-preloader")
          .remove();
        ...
        var pagination = 1;
        var control_for = "";
        var elementClasses = $(".fp-preloader-hr-carousel_" + index)
          .attr("class")
          .split(" ");
        for (var i = 0; i < elementClasses.length; i++) {
          if (elementClasses[i].indexOf("visible-") != -1)
  ...
  var counters = function () {
    $(".counters-group").each(function () {
      ...
      counterBoxes.each(function (index) {
        ...
        self
          .find(".counter-box-path")
          .data("dashoffset", 439 - (currentValue / topValue) * 439);
      });
    });
  ...
  $(".accordion").accordion({
    ...
    create: function (event, ui) {
      ...
      $(".horizontal-carousel").trigger("configuration", [
        "debug",
        false,
        true,
      ]);
  ...
  function windowResize() {
    ...
    $(".horizontal-carousel").each(function () {
      ...
      self.data(
        "visible",
        visible == 1 || $(".header").width() <= 462
          ? self.hasClass("features-list") && self.hasClass("type-big")
            ? $(".header").width() <= 300
              ? 2
              : 3
            : 1
        ...
        if (menu_position + modificator < topOfWindow) {
          if (!$("#fp-sticky-clone").hasClass("move")) {
            ...
            if ($(".transparent-header-container").length) {
              $('.header-container.sticky:not("#fp-sticky-clone")').css(
                "visibility",
                "hidden"
              );
              setTimeout(function () {
  ...
  $("[data-plan]").on("click", function (event) {
    ...
    if (self.attr("href").length) {
      $(self.attr("href")).slideDown(200, function () {
        $("html, body").animate(
          { scrollTop: $(self.attr("href")).offset().top - 120 },
          400
        );
      });
    }
  ...
  if ($("form.cost-calculator-container").length) {
    ...
    $("form.cost-calculator-container .more-submit").on(
      "click",
      function (event) {
        event.preventDefault();
  ...
  $(".contact-form").on("submit", function (event) {
    ...
    if (
      typeof self.data("append") != "undefined" &&
    ...
    $.ajax({
      ...
      success: function (json) {
        self
          .find(
            ".submit-contact-form, [name='submit'], [name='name'], [name='email'], [name='phone'], [name='subject'], [name='message']"
          )
          .qtip("destroy");
        if (typeof json.isOk != "undefined" && json.isOk) {
          if (
            ...
            if (
              ...
              $("#" + self.data("append") + " [data-plan]").removeClass(
                "selected"
              );
              $("#" + self.data("append") + " .cost-slider-input").trigger(
                "change"
              );
              $("#" + self.data("append") + " .cost-dropdown").selectmenu(
                "refresh"
              );
              $(
                "#" +
          ...
          if (
            typeof json.error_subject != "undefined" &&
            json.error_subject != ""
          ) {
  ...
  if ($(".header-container").hasClass("sticky")) {
    ...
    $(".header-container.sticky").after(
      $(".header-container.sticky").clone().attr("id", "fp-sticky-clone")
    );
  }
  function animateElements() {
    $(
      '.animated-element, .header-container.sticky:not("#fp-sticky-clone"), .fp-smart-column, .has-counter-box-path'
    ).each(function () {
        ...
        if (
          ...
          if (typeof self.data("steps") != "undefined") {
            self
              ...
              .css(
                "width",
                (parseInt(self.data("step"), 10) /
                  parseInt(self.data("steps"), 10)) *
                  100 +
                  "%"
              );
            self.css(
              "left",
              (100 / parseInt(self.data("step"), 10)) *
                (parseInt(self.data("step"), 10) - 1) +
                "%"
            );
          ...
          ($(this).hasClass("counter-box-path") ||
            (isSafari && $(this).hasClass("has-counter-box-path"))) &&
          !$(this).hasClass("progress") &&
          $(this).is(":visible")
        ) {
          var self = isSafari
            ? $(this).children("svg").children().first()
            : $(this);
          self.addClass("progress");
          var dashoffset = self.data("dashoffset");
          var duration =
            typeof self.data("duration") != "undefined"
              ? self.data("duration")
              : 2000;
          self.animate(
            {
          ...
          !$(this).hasClass("progress") &&
          !$(this).hasClass("has-counter-box-path")
        ) {
          var elementClasses = $(this).attr("class").split(" ");
      ...
      mutations.forEach(function (mutationRecord) {
        ...
        var progressBarInterval = setInterval(function () {
          $(mutationRecord.target)
            .next()
            .css("max-width", 100 + intervalCounter + "%");
          intervalCounter++;
